...
Notation "f 〈 F ∣ B 〉" := (Transform F f B)
  (at level 5, F at level 0, B at level 0) : filter_scope.
...
Definition AlmostEqual f g A B F0 := Function f /\ Function g
  /\ dom(f) = A /\ dom(g) = A /\ ran(f) ⊂ B /\ ran(g) ⊂ B
  /\ F0 ∈ (β A) /\ \{ λ u, u ∈ A /\ f[u] = g[u] \} ∈ F0.
...
Definition Arithmetical_ultraFilter F0 A := ~ Finite A /\ F0 ∈ (β A)
  /\ (∀ f g, Function f -> Function g
    -> dom(f) = A -> dom(g) = A -> ran(f) ⊂ A -> ran(g) ⊂ A
    -> f〈F0∣A〉 = g〈F0∣A〉 -> AlmostEqual f g A A F0).
