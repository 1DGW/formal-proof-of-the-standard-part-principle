...
(* 自反性 *)
Definition Reflexivity R a := ∀ x, x ∈ a -> Rrelation x R x.

(*定义: R在a上是对称的*)
Definition Symmetry R a := ∀ x y, x ∈ a -> y ∈ a
  -> Rrelation x R y -> Rrelation y R x.

(*定义: R在a上是传递的
       此定义MK中实际上已有 (Transitive) *)
Definition Transitivity R a := ∀ x y z, x ∈ a -> y ∈ a -> z ∈ a
  -> Rrelation x R y -> Rrelation y R z -> Rrelation x R z.
...
(*定义: 等价类, 关于 a上的等价关系R的以x为代表的 等价类.
       [注: 1.实际使用此定义时, 要求x为a的一个元, R为a上的一个等价关系.
            2.当R为一个等价关系时, 'xRy'除了原有的'x,y满足R关系'之意,
              也表达了'x与y是关于R等价的'. ] *)
Definition equClass x R a := \{ λ u, u ∈ a /\ Rrelation u R x \}.

(*关于等价类的定理1: 相互等价的元素所代表的等价类相等. *)
Theorem equClassT1 : ∀ R a x y, equRelation R a -> x ∈ a -> y ∈ a
  -> Rrelation x R y <-> equClass x R a = equClass y R a.
Proof.
  split; intros.
...
Definition QuotientSet a R := \{ λ u, ∃ x, x ∈ a
  /\ u = equClass x R a \}.

Declare Scope ec_scope.
Delimit Scope ec_scope with ec.
Open Scope ec_scope.

Notation "a / R" := (QuotientSet a R) : ec_scope.
